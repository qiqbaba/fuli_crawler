...
class JingpinCrawler(DecryptSiteBaseCrawler):
    ...
    def __init__(self, db_manager):
        config = CrawlConfig(
            source_name="jingpin",
            categories=["guochan", "oumei"],
            initial_domains=["kyy.282969.xyz", "fes.399695.xyz", "uah.292368.xyz"],
            main_domain="https://jpbt3.com",
            domain_pattern=r'([a-z0-9]{2,10}\.\d{5,7}\.xyz)',
        )
        super().__init__(db_manager, "jingpin", config=config)
    ...
    def _is_valid_list_page(self, html):
        ...
        return 'class="movie_list"' in html or 'class="bt_ul"' in html or 'class="list"' in html or "class='list'" in html or "list.php" in html
    ...
    def parse_list_page(self, list_page_content, page_num):
        ...
        # 1. 优先按传统类名查找
        ul = soup.find('ul', class_='bt_ul') or soup.find('ul', class_='list')
    ...
    def _should_rewrite_url(self, netloc):
        """判断是否应使用当前域名重写 URL"""
        if "jpbt3.com" in self.base_domain:
            return False

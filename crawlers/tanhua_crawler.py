...
class TanhuaCrawler(DecryptSiteBaseCrawler):
    ...
    def __init__(self, db_manager):
        config = CrawlConfig(
            source_name="tanhua",
            categories=["guochan", "oumei"],
            initial_domains=["urp.282969.xyz", "sij.292368.xyz", "uns.399695.xyz"],
            main_domain="https://thbt8.com",
            domain_pattern=r'([a-z0-9]{2,10}\.\d{5,7}\.xyz)',
        )
        super().__init__(db_manager, "tanhua", config=config)
    ...
    def _is_valid_list_page(self, html):
        ...
        return 'class="movie_list"' in html or 'class="videos-list"' in html or 'class="list"' in html or "class='list'" in html or 'class="bt_ul"' in html or "list.php" in html
    ...
    def parse_list_page(self, list_page_content, page_num):
        ...
        # 2. 适配新版结构：从 <div class="movie_list"> 或 <div class="videos-list"> 容器查找 <ul>
        if not ul:
            container = soup.find('div', class_='movie_list') or soup.find('div', class_='videos-list')
            if container:
    ...
    def _should_rewrite_url(self, netloc):
        """判断是否应使用当前域名重写 URL"""
        if "thbt8.com" in self.base_domain:
            return False

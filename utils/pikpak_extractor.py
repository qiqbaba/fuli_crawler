...
def get_pikpak_link(keepshare_url, timeout=30, poll_interval=2):
    """
    ...
    print(f"[*] 开始请求 KeepShare 链接: {keepshare_url}")
    try:
        # 发送 GET 请求，禁止自动跳转
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    if api_res.status_code == 200:
                        ...
                        print(f"    [轮询] 当前状态: {state} | 链接: {pikpak_link or '等待生成中...'}")
                        
                        if pikpak_link:
                            print(f"[OK] 资源转存成功，轮询获取到 PikPak 链接！花费时间: {int(time.time() - start_time)}秒")
                            return pikpak_link
                            
                        # 如果出现 ERROR 状态
                        if state == "ERROR":
                            print(f"[-] 转存任务出错，错误原因: {data.get('error')}")
                            return None
                    else:
                        print(f"    [警告] API 请求失败，状态码: {api_res.status_code}")
                except Exception as api_err:
                    print(f"    [警告] 轮询请求发生异常: {api_err}")
                    
                time.sleep(poll_interval)
                
            print(f"[-] 轮询超时 ({timeout}秒)，资源可能在排队离线中，您可以稍后直接请求该 KeepShare 链接。")
            return None
            
        # 其他不认识的重定向
        print(f"[-] 遇到未知的重定向目标: {location}")
        return None

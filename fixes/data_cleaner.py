...
def find_dirty_link_records(conn) -> List[Tuple[int, str, str, str]]:
...
def sync_supabase_links(changes: List[Tuple[int, str, str, str]]) -> None:
    ...
    for idx, (row_id, url, old_link, new_link) in enumerate(changes, 1):
        ...
        try:
            ...
            failed += 1
            print(f"[-] Supabase 更新失败 (url={url[:60]}...): {e}")
        if idx % 200 == 0:
...
def run_clean_noise(args):
    """清洗 resource_link 中的广告与标签行"""
    db_path = get_db_path(getattr(args, "db", None))
    print("=" * 60)
    print("         清理 resource_link 残留标签行与推广噪音")
    print("=" * 60)
    print(f"[*] 运行模式: {'【正式执行模式】' if getattr(args, 'run', False) else '【预览模式 (Dry Run)】'}")
    ...
    if not getattr(args, "run", False):
        ...
        print("[*] 确认无误后请运行: python fixes/data_cleaner.py clean-noise --run")
        conn.close()
        return
...
def run_replace_domain(args):
    """批量替换 URL 中的指定域名或子串"""
    db_path = get_db_path(getattr(args, "db", None))
    old_val = getattr(args, "old", None) or "dyh.393659.xyz"
    new_val = getattr(args, "new", None) or "dtn.628563.xyz"
    is_run = getattr(args, "run", False)

    print("=" * 60)
    print("                 批量替换 URL 域名")
    print("=" * 60)
    print(f"[*] 运行模式: {'【正式执行模式】' if is_run else '【预览模式 (Dry Run)】'}")
    print(f"[*] 数据库路径: {db_path}")
    ...
    rows = cursor.fetchall()
    print(f"[*] 找到 {len(rows)} 条包含 '{old_val}' 的记录:")
    ...
    if not is_run:
        print("\n[*] 当前为预览模式，未修改数据库。")
        print(f"[*] 确认替换请运行: python fixes/data_cleaner.py replace-domain --old \"{old_val}\" --new \"{new_val}\" --run")
        conn.close()
        return
...
def upgrade_single_database(db_path: str) -> bool:
    """升级单个数据库的表结构并提取填入元数据"""
    print(f"\n================ 开始处理数据库: {db_path} ================")
    if not os.path.exists(db_path):
    ...
    try:
        ...
        if need_migration:
            print("  [*] 检测到表结构需要升级重构，正在备份并执行数据迁移...")
            ...
            conn.commit()
            print("  [+] 成功迁移表结构并对齐目标字段！")
        else:
            print("  [*] 数据库表结构已是最新，无需迁移结构。")
    ...
    # 提取并更新历史元数据
    print("  [*] 正在扫描历史数据进行元数据提取（size/format/pikpak）...")
    ...
    if update_data:
        ...
        conn.commit()
        if total_records > 0:
            print(f"  [+] 批量更新完成！成功解析填入 size/format: {matched_count}/{total_records} ({matched_count/total_records:.1%})")
            print(f"  [+] 成功解析填入 PikPak 链接: {pikpak_count}/{total_records}")
    conn.close()
    return True


def run_upgrade_db(args):
    """升级数据库结构与元数据"""
    target_db = getattr(args, "db", None)
    paths = [target_db] if target_db else DB_PATHS
    updated_any = False
    for path in paths:
        if upgrade_single_database(path):
            updated_any = True
    if not updated_any:
        print("\n[-] 未找到任何有效数据库文件，请检查配置。")
...
def _darklyn_run_fetch(db_path: str, limit: int = 0, concurrency: int = 3, retry: bool = False) -> int:
    ...
    _darklyn_done = 0
    print(f"待处理 {_darklyn_total} 条（已跳过 {len(hashes) - _darklyn_total} 条）")
    _darklyn_log(f"开始: 待处理 {_darklyn_total} 条, 并发 {concurrency}")

    if not todo:
        ok = sum(1 for v in _darklyn_results.values() if v.get("size"))
        print(f"\n无需抓取: 已命中 {ok} / {len(_darklyn_results)} ({(100.0 * ok / max(len(_darklyn_results), 1)):.1f}%)")
        return ok
    ...
    _save_darklyn_results()
    ok = sum(1 for v in _darklyn_results.values() if v.get("size"))
    print(f"\n完成: 命中 {ok} / {len(_darklyn_results)} ({(100.0 * ok / max(len(_darklyn_results), 1)):.1f}%)")
    return ok
...
def run_fetch_sizes(args):
    ...
    apply_db = getattr(args, "apply", False)
...
def run_fetch_empty_links(args):
    ...
    print("=" * 60)
    print("        Playwright 重新抓取回填空 resource_link")
    print("=" * 60)
    print(f"[*] 数据库路径: {db_path}")
    ...
    if domain_filter:
        ...
        print(f"[*] 匹配域名 '{domain_filter}' 的待处理记录数: {len(records)}")
    ...
    try:
        with sync_playwright() as p:
            ...
            for idx, (db_id, url, title) in enumerate(records, 1):
                print(f"[{idx}/{len(records)}] 正在处理: {(title or '')[:25]}... ID: {db_id}")
                try:
...
def interactive_menu():
    ...
    try:
        choice = input("请输入序号 [0-5] (直接回车退出): ").strip()
    ...
    if choice == "1":
        confirm_run = input("是否正式执行写入？[y/N] (直接回车仅预览): ").strip().lower()
        setattr(args, "run", confirm_run == "y")
        setattr(args, "sync_supabase", False)
        setattr(args, "sync_dynamodb", False)
        if confirm_run == "y":
            sync_sup = input("是否同时同步到 Supabase？[y/N]: ").strip().lower()
            setattr(args, "sync_supabase", sync_sup == "y")
            sync_dyn = input("是否同时同步到 DynamoDB？[y/N]: ").strip().lower()
            setattr(args, "sync_dynamodb", sync_dyn == "y")
        ...
        confirm_run = input("是否正式执行替换？[y/N] (直接回车仅预览): ").strip().lower()
        ...
        confirm = input("确定要升级数据库表结构并重新提取元数据？[y/N]: ").strip().lower()
        if confirm == "y":
            run_upgrade_db(args)
        else:
            print("[*] 已取消操作。")

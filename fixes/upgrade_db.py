...
def upgrade_database(db_path):
    ...
    try:
        ...
        if not columns_info:
            logger.warning("  -> 未找到 resources 表，可能尚未初始化。")
            conn.close()
            return False
        ...
        if need_migration:
            logger.info("  -> 检测到表结构需要升级重构，正在进行数据迁移...")
            # 开启事务
    ...
    if update_data:
        ...
        conn.commit()
        if total_records > 0:
            logger.info(f"  -> 批量更新完成！成功解析并填入有效元数据共 {matched_count}/{total_records} 条记录 ({matched_count/total_records:.2%})。")
            logger.info(f"  -> 成功提取并填入 PikPak 链接共 {pikpak_count}/{total_records} 条记录。")
        else:
